...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs)
{
	...
	if ((sig_grs >> (23 + 3)) > 1 || exp < 0)
	{
		...
		uint64_t sticky = 0x0;
		while ((((sig_grs >> (23 + 3)) > 1) && exp < 0xff) // condition 1 for addition
			   ||										   // or
			   (sig_grs > 0x04 && exp < 0)				   // condition 2 for mulplication
			   )
		{
		...
		if (exp >= 0xff)
		{
			...
			// assign sig_grs to 0x0 && exp to 0xff
			// sign doesn't change
			sig_grs = 0x0;
			exp = 0xff;
			overflow = true;
	...
	if (!overflow)
	{
		...
		// now sig_grs is 24 bits 
		// if sig_grs's hidden bit is more than 1
		if ((sig_grs >> 23) > 1){
		    exp++;
		    sig_grs  = sig_grs >> 1;
		}
...
uint32_t get_shift(uint32_t a_exponent, uint32_t b_exponent){
    int32_t true_exp_a, true_exp_b;
    if (a_exponent == 0){
        true_exp_a = -126;
    }else{
        true_exp_a = (int32_t) a_exponent - 127;
    }
    if (b_exponent == 0){
        true_exp_b = -126;
    }else{
        true_exp_b = (int32_t)(b_exponent - 127);
    }
    uint32_t shift = true_exp_b - true_exp_a;
    return shift;
...
uint32_t internal_float_add(uint32_t b, uint32_t a)
{
	...
	uint32_t shift = get_shift(fa.exponent, fb.exponent);
	assert(shift >= 0);
...
uint32_t internal_float_mul(uint32_t b, uint32_t a)
{
	...
	/*uint32_t my_ans = internal_normalize(f.sign, exp_res, sig_res);
	FLOAT golden_ans;
	golden_ans.fval = fa.fval * fb.fval;
	printf("a: 0x%x, b: 0x%x\n", fa.val, fb.val);
	printf("my ans is 0x%x\n", my_ans);
	printf("golden ans is 0x%x\n", golden_ans.val);
	fflush(stdout);*/
	return internal_normalize(f.sign, exp_res, sig_res);
}
